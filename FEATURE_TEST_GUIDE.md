...
本指南面向首次克隆 `nekoneko` demo snapshot 的体验者，用于启动项目、了解当前功能，并按统一步骤验证主要交互、Agent 路由、安全边界与研究数据页面。

当前同步基线：QAQ `main` 提交 `70adb29`。
...
默认配置为 `DEMO_MODE=true`，使用 fake/mock provider，不需要 API key。默认模式适合离线检查 UI、路由、安全、记忆、提醒、传感器和导出功能。
...
| 10 | 提醒管理 | 输入“每天晚上7点半提醒我散步”，再打开“提醒”页面 | 出现每日提醒；可试触发、确认和删除 |
...
| 12 | 查询、安全、摘要与导出 | 按第 3 节的测试语句和页面操作 | 验证受控检索、安全拒绝、照护摘要和评估数据导出 |
...
- [评估页面](http://localhost:3000/evaluation)：查看 Route/Risk 统计，并下载 JSON 或 CSV。

评估导出只包含 Trace 元数据和聚合统计，不包含完整聊天记录。
